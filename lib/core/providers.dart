...
final storageServiceProvider = Provider<StorageService>((ref) {
  throw UnimplementedError(
    'StorageService must be overridden in main',
  );
});
...
final adsbServiceProvider = Provider<AdsbService>((ref) {
  return AdsbService();
});
...
class SettingsNotifier extends StateNotifier<AppSettings> {
  ...
  Future<void> update(
    AppSettings Function(AppSettings) fn,
  ) async {
    final next = fn(state);
  ...
  Future<void> setLockEnabled(bool value) =>
      update((s) => s.copyWith(lockEnabled: value));

  Future<void> setBiometricEnabled(bool value) =>
      update((s) => s.copyWith(biometricEnabled: value));

  Future<void> setAlertsEnabled(bool value) =>
      update((s) => s.copyWith(alertsEnabled: value));

  Future<void> setPrivacyMode(bool value) =>
      update((s) => s.copyWith(privacyMode: value));
  ...
  Future<void> setMissionMode(bool value) =>
      update((s) => s.copyWith(missionMode: value));
}
...
final currentDayNumberProvider = Provider<int>((ref) {
  ...
  return ChelehDateUtils.calculateCurrentDay(
    settings.chelehStartDate,
  );
});

final todayChelehProvider =
    StateNotifierProvider<TodayChelehNotifier, AsyncValue<ChelehDay>>(
  (ref) {
    return TodayChelehNotifier(ref);
  },
);

class TodayChelehNotifier
    extends StateNotifier<AsyncValue<ChelehDay>> {
  final Ref _ref;

  TodayChelehNotifier(this._ref)
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (day == null) {
        final start =
            settings.chelehStartDate ?? DateTime.now();

        final date = ChelehDateUtils.dateForDay(
          start,
          dayNum,
        );
  ...
  Future<void> toggleNamaz(bool value) =>
      _update(
        (d) => d.copyWith(
          namazShab: value,
        ),
  ...
  Future<void> toggleZiyarat(bool value) =>
      _update(
        (d) => d.copyWith(
          ziyaratAshura: value,
        ),
  ...
  Future<void> toggleTavassol(bool value) =>
      _update(
        (d) => d.copyWith(
          doayeTavassol: value,
        ),
  ...
  Future<void> _update(
    ChelehDay Function(ChelehDay) fn,
  ) async {
...
final progressProvider =
    Provider<({int completed, int total, double percent})>(
  (ref) {
    final storage = ref.watch(storageServiceProvider);

    final completed =
        storage.countCompletedDays();

    return (
      completed: completed,
      total: 40,
      percent: ChelehDateUtils.progressPercent(
        completed,
      ),
    );
  },
...
final allDaysProvider =
    FutureProvider<List<ChelehDay>>((ref) async {
  final storage = ref.watch(storageServiceProvider);
...
final aircraftListProvider = StateNotifierProvider<
    AircraftNotifier,
    AsyncValue<List<AircraftInfo>>>(
  (ref) {
    return AircraftNotifier(ref);
  },
);

class AircraftNotifier
    extends StateNotifier<AsyncValue<List<AircraftInfo>>> {
  final Ref _ref;

  AircraftNotifier(this._ref)
